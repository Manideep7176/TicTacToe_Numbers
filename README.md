...
Defining the action space for each state. (Being careful in defining actions. The actions are not the same for each state)

Defining the winning states: the sum of three numbers in a row, column or diagonal is 15.

Defining the terminal states (win,tie,loss)

Building the reward structure as below:
